...
val repo = TextRepositorySource.fromCexFile("texts/horace.cex")
val books = repo.corpus.nodes.map(_.urn.collapsePassageTo(1)).distinct
val poems = repo.corpus.nodes.map(_.urn.collapsePassageTo(2)).distinct
...
def tokenize(corpus: Corpus, units: Vector[CtsUrn] = Vector.empty[CtsUrn], unitLabel: String = "poem") :  Vector[LatinToken]= {
